    ...
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (DateTime.TryParse(textBox1.Text, out DateTime targetDate))
            {
                ...
                // Выводим результат
                label1.Text = $"От текущей даты {result:F2} {(radioButtonYears.Checked ? "год(а)" : radioButtonMonths.Checked ? "месяц(ев)" : radioButtonDays.Checked ? "день(дней)" : radioButtonMinutes.Checked ? "минут(ы)" : "секунд(ы)")} до указанной даты.";
            }
            else
            {
